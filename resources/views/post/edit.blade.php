...
<div class="container">
    ...
    <div class="row">
      ...
      <form action="{{route('post.update',$post->id)}}" method="post" class="form-group" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <label>Enter Title</label><input type="text" name="title" value="{{$post->title}}" class="form-control" /><br>
        <label>Enter Content</label><textarea name="content" class="form-control">{{$post->body}}</textarea><br>
        ...
        <img src="{{asset($post->image)}}" width="200px"/><br>
